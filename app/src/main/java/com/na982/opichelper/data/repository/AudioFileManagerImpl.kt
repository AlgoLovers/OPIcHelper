...
class AudioFileManagerImpl(private val context: Context) : AudioFileManager {
    ...
    companion object {
        ...
        private const val DEBUG_MERGE = false // 병합 과정 상세 로그 비활성화
        private const val DEBUG_FILE_OPERATIONS = false // 파일 작업 상세 로그 비활성화
    }
    
    override suspend fun mergeAndSaveAudioFiles(files: List<File>, scriptId: String): File? {
        return withContext(Dispatchers.IO) {
            if (files.isEmpty()) {
                Log.d(TAG, "병합할 파일이 없습니다.")
                return@withContext null
            }

            Log.i(TAG, "병합 시작: ${files.size}개 파일")
            if (DEBUG_MERGE) {
                files.forEachIndexed { idx, file ->
                    Log.d(TAG, "파일 ${idx + 1}: ${file.name} (${file.length()} bytes)")
                }
            }
            ...
            Log.d(TAG, "출력 파일: ${output.name}")

            // 단일 파일인 경우 그대로 복사
            if (files.size == 1) {
                files[0].copyTo(output, overwrite = true)
                Log.i(TAG, "단일 파일 복사 완료: ${output.length()} bytes")
            ...
            try {
                ...
                Log.i(TAG, "MediaCodec 병합 완료: ${output.length()} bytes")
            } catch (e: Exception) {
                Log.w(TAG, "MediaCodec 병합 실패, 헤더 분석 방식 사용", e)
                try {
                    mergeWithHeaderAnalysis(files, output)
                    Log.i(TAG, "헤더 분석 병합 완료: ${output.length()} bytes")
                } catch (e2: Exception) {
                    Log.w(TAG, "헤더 분석 병합 실패, fallback 방식 사용", e2)
                    // Fallback: 간단한 파일 연결
                    FileOutputStream(output).use { out ->
                        files.forEachIndexed { idx, file ->
                            if (DEBUG_MERGE) {
                                Log.d(TAG, "파일 ${idx + 1} 처리: ${file.name}")
                            }
                            FileInputStream(file).use { input ->
                                val bytesCopied = input.copyTo(out)
                                totalBytesWritten += bytesCopied
                                if (DEBUG_MERGE) {
                                    Log.d(TAG, "파일 ${idx + 1} 복사 완료: $bytesCopied bytes")
                                }
                            }
                        }
                    }
                    
                    Log.i(TAG, "Fallback 병합 완료: 총 ${totalBytesWritten} bytes")
                }
            }
            ...
            files.forEach { file ->
                if (file.exists()) {
                    ...
                    if (DEBUG_FILE_OPERATIONS) {
                        Log.d(TAG, "원본 파일 삭제: ${file.name}, 성공: $deleted")
                    }
                }
            }
            ...
            if (output.exists() && output.length() > 0) {
                Log.i(TAG, "병합 완료: ${output.name} (${output.length()} bytes)")
            } else {
                Log.e(TAG, "병합 실패: 파일=${output.exists()}, 크기=${output.length()}")
            }
        ...
        withContext(Dispatchers.IO) {
            try {
                ...
                if (!sourceFile.exists()) {
                    Log.e(TAG, "녹음 파일이 존재하지 않음: $recordedFile")
                    return@withContext
                }
                ...
                Log.d(TAG, "녹음 파일 저장: ${destinationFile.name}")

                // 원본 파일 삭제
                sourceFile.delete()
            } catch (e: Exception) {
                Log.e(TAG, "녹음 파일 저장 실패", e)
            }
        }
    }
    ...
    override fun deleteAudioFile(file: File) {
        if (file.exists()) {
            val deleted = file.delete()
            if (DEBUG_FILE_OPERATIONS) {
                Log.d(TAG, "오디오 파일 삭제: ${file.name}, 성공: $deleted")
            }
        }
    }
    ...
    override suspend fun cleanupOldRecordings(scriptId: String, keepLatestCount: Int) {
        withContext(Dispatchers.IO) {
            ...
            scriptFiles?.drop(keepLatestCount)?.forEach { file ->
                val deleted = file.delete()
                if (DEBUG_FILE_OPERATIONS) {
                    Log.d(TAG, "오래된 녹음 파일 삭제: ${file.name}, 성공: $deleted")
                }
            }
        }
    }
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                if (file.exists()) {
                    val deleted = file.delete()
                    if (DEBUG_FILE_OPERATIONS) {
                        Log.d(TAG, "녹음 파일 삭제: ${file.name}, 성공: $deleted")
                    }
                    deleted
                } else {
                    if (DEBUG_FILE_OPERATIONS) {
                        Log.d(TAG, "삭제할 녹음 파일이 존재하지 않음: ${file.name}")
                    }
                    false
                }
            } catch (e: Exception) {
                Log.e(TAG, "녹음 파일 삭제 실패: ${File(filePath).name}", e)
                false
            }
        }
    }
    ...
    override suspend fun saveRecordingFile(recordingFile: File, fileName: String): File {
        return withContext(Dispatchers.IO) {
            ...
            Log.d(TAG, "녹음 파일 저장: ${outputFile.name} (${outputFile.length()} bytes)")
            outputFile
        }
    }
    ...
    override suspend fun mergeAudioFiles(files: List<File>, mergedFileName: String): File {
        return withContext(Dispatchers.IO) {
            ...
            Log.i(TAG, "오디오 파일 병합 시작: ${mergedFileName}")
            
            if (files.size == 1) {
                files[0].copyTo(outputFile, overwrite = true)
                Log.i(TAG, "단일 파일 복사 완료")
            } else {
                mergeWithMediaCodec(files, outputFile)
                Log.i(TAG, "MediaCodec 병합 완료")
            }
            
            Log.i(TAG, "오디오 파일 병합 완료: ${outputFile.name} (${outputFile.length()} bytes)")
    ...
    override suspend fun hasEnglishWritingTestMergedFile(category: String, scriptIndex: Int): Boolean {
        return withContext(Dispatchers.IO) {
            ...
            if (!mergedDir.exists()) {
                if (DEBUG_FILE_OPERATIONS) {
                    Log.d(TAG, "영작테스트 파일 확인: merged 디렉토리가 존재하지 않음")
                }
            ...
            if (DEBUG_FILE_OPERATIONS) {
                Log.d(TAG, "영작테스트 파일 확인: $category/$scriptIndex = $hasFile")
            }
            hasFile
        }
    }
    ...
    override suspend fun getEnglishWritingTestMergedFile(category: String, scriptIndex: Int): File? {
        return withContext(Dispatchers.IO) {
            ...
            if (!mergedDir.exists()) {
                if (DEBUG_FILE_OPERATIONS) {
                    Log.d(TAG, "영작테스트 파일 조회: merged 디렉토리가 존재하지 않음")
                }
                return@withContext null
            }
            ...
            if (DEBUG_FILE_OPERATIONS) {
                Log.d(TAG, "영작테스트 파일 조회: $category/$scriptIndex = ${result?.name}")
            }
            result
        }
    }
    ...
    override suspend fun hasFullMemorizationRecording(category: String, scriptIndex: Int): Boolean {
        return withContext(Dispatchers.IO) {
            ...
            if (!recordingsDir.exists()) {
                if (DEBUG_FILE_OPERATIONS) {
                    Log.d(TAG, "통암기 파일 확인: recordings 디렉토리가 존재하지 않음")
                }
                return@withContext false
            }
            ...
            val hasFile = files?.isNotEmpty() == true
            if (DEBUG_FILE_OPERATIONS) {
                Log.d(TAG, "통암기 파일 확인: $category/$scriptIndex = $hasFile")
            }
            hasFile
        }
    }
    ...
    override suspend fun getFullMemorizationRecording(category: String, scriptIndex: Int): File? {
        return withContext(Dispatchers.IO) {
            ...
            if (!recordingsDir.exists()) {
                if (DEBUG_FILE_OPERATIONS) {
                    Log.d(TAG, "통암기 파일 조회: recordings 디렉토리가 존재하지 않음")
                }
                return@withContext null
            }
            ...
            val result = files?.maxByOrNull { it.lastModified() }
            if (DEBUG_FILE_OPERATIONS) {
                Log.d(TAG, "통암기 파일 조회: $category/$scriptIndex = ${result?.name}")
            }
            result
        }
    }
