...
class GoogleTtsPlayer(private val context: Context) : TtsPlayer {
    ...
    private fun initializeTts() {
        tts = TextToSpeech(context) { status ->
            if (status == TextToSpeech.SUCCESS) {
                ...
                Log.d("GoogleTtsPlayer", "Google TTS 초기화 완료: $isInitialized")
            } else {
                Log.e("GoogleTtsPlayer", "Google TTS 초기화 실패")
            }
        }
    }
    ...
    override suspend fun speak(text: String, onComplete: (() -> Unit)?): Boolean {
        return if (isAvailable()) {
            try {
                Log.d("GoogleTtsPlayer", "Google TTS 시작: $text")
                isPlaying = true
                
                val utteranceId = "google_tts_${System.currentTimeMillis()}"
                ...
                tts?.setOnUtteranceProgressListener(object : UtteranceProgressListener() {
                    ...
                    override fun onDone(utteranceId: String?) {
                        Log.d("GoogleTtsPlayer", "Google TTS 재생 완료")
                        isPlaying = false
                        onComplete?.invoke()
                    }
                    @Deprecated("Deprecated in Android API")
                    override fun onError(utteranceId: String?) {
                        Log.e("GoogleTtsPlayer", "Google TTS 재생 오류")
                        isPlaying = false
                        onComplete?.invoke()
                    }
                    override fun onError(utteranceId: String?, errorCode: Int) {
                        Log.e("GoogleTtsPlayer", "Google TTS 재생 오류: $errorCode")
                        isPlaying = false
                        onComplete?.invoke()
                    }
                })
                ...
                Log.e("GoogleTtsPlayer", "Google TTS 오류", e)
                isPlaying = false
                onComplete?.invoke()
                false
            }
        } else {
            Log.e("GoogleTtsPlayer", "Google TTS 사용 불가")
            onComplete?.invoke()
            false
        }
    }
    ...
    override fun stop() {
        tts?.stop()
        isPlaying = false
        Log.d("GoogleTtsPlayer", "Google TTS 중지")
    }
