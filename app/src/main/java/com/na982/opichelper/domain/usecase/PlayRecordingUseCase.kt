...
class PlayRecordingUseCase @Inject constructor(
        ...
        try {
            when (memorizeLevel) {
                MemorizeLevel.ENGLISH_WRITING -> {
                    playEnglishWritingRecording(category, scriptIndex, onHighlight, onCompletion)
                }
                MemorizeLevel.FULL_MEMORIZATION -> {
                    playFullMemorizationRecording(category, scriptIndex, onHighlight, onCompletion)
                }
    ...
    private suspend fun playEnglishWritingRecording(
        ...
        if (recordingPath != null) {
            ...
            recordingAudioPlayer.playRecording(recordingPath, onHighlight) {
                Log.d("PlayRecordingUseCase", "영작테스트 녹음 재생 완료")
                onCompletion()
            }
        } else {
    ...
    private suspend fun playFullMemorizationRecording(
        ...
        if (recordingPath != null) {
            ...
            // 녹음 파일 재생
            recordingAudioPlayer.playRecording(recordingPath, onHighlight) {
                Log.d("PlayRecordingUseCase", "통암기 녹음 재생 완료")
                onCompletion()
            }
        } else {
