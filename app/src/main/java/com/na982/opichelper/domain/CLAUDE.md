...
| `ScriptProgress.kt` | 진행상황 상태. 필드: category, scriptIndex, memorizeLevel, currentSentenceIndex, totalSentences, isMemorizeTestRunning, timestamp, needsSave. getKey(), toPersistable() 포함 |
| `CurrentMode.kt` | 코디네이터 모드 상태 (NONE, REPEAT_LISTENING, ENGLISH_WRITING, FULL_MEMORIZATION_*). group 프로퍼티로 ModeGroup 반환 |
| `ModeGroup.kt` | 모드 그룹 (NONE, REPEAT_LISTENING, ENGLISH_WRITING, FULL_MEMORIZATION) |

**주의**: `ProgressData` data class는 이 파일이 아닌 `EnglishWritingTestRepository.kt`에 별도 정의됨 (필드: category, scriptIndex, memorizeLevel, currentSentenceIndex, totalSentences, isMemorizeTestRunning). 파일명과 클래스명 혼동 주의.
